...
export default function CourseMapPage({ parts, allSteps, completed, started, lastLesson, continueStep, onReset, query, setQuery, product = 'tas', t }) {
  ...
  return <div className="course-map-page">
    <section className="course-map-hero"><div><span className="eyebrow">{product.toUpperCase()} · {t.courseOverview}</span><h1>{t.heroProduct} <em>{t.heroTitle}</em></h1><p>{t.heroIntro}</p><a className="primary-button" href={getStepPath(continueStep, product)}><Play size={17} />{t.continueLearning}</a></div><aside><span>{t.overallProgress}</span><strong>{percentage}%</strong><ProgressBar completed={completed.size} total={allSteps.length} t={t} /><small>{t.stepsProgress(completed.size, allSteps.length)}</small></aside></section>
