...
export default function CompletionChecklist({ checks, selectedChecks, isComplete, onCheck, onToggle, step, destination, t }) {
  ...
  if (isComplete) {
    ...
    const productLabel = getProductLabel(destination.product)
    const nextTitle = destination.type === 'course' ? `Cubicost ${productLabel}` : destination.type === 'part' ? t.nextPart(destination.nextPartNumber, destination.nextPart) : destination.nextLesson.title
    return <section className="completion-card completion-success" id="lesson-completion" aria-live="polite" tabIndex="-1" ref={successRef}><span className="completion-success__icon"><Check size={20} /></span><div><h2>{heading}</h2><p>{destination.type === 'course' ? t.completedCourse(productLabel) : destination.type === 'part' ? t.completedPart(destination.completedPart) : t.completedLesson(step.title)}</p><small>{t.nextDestination}</small><strong>{nextTitle}</strong></div><a className="primary-button" href={destination.href}>{action}<ArrowRight size={17} /></a></section>
  }
